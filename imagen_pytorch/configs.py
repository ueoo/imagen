...
from imagen_pytorch.imagen_pytorch import Imagen, NullUnet, Unet, Unet3D
...
class ImagenConfig(AllowExtraBaseModel):
    ...
    def check_image_sizes(self):
        if len(self.image_sizes) != len(self.unets):
            raise ValueError(
                f"image sizes length {len(self.image_sizes)} must be equivalent to the number of unets {len(self.unets)}"
            )
        return self
...
class ImagenTrainerConfig(AllowExtraBaseModel):
    ...
    def create(self):
        ...
        imagen = imagen_config_klass(**{**imagen_config, "video": self.video}).create()
