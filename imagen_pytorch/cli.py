...
def train(
    ...
    if "channels" in config_data["imagen"]:
        assert (
            config_data["imagen"]["channels"] > 0 and config_data["imagen"]["channels"] < 5
    ...
    for i in range(epoches):
        ...
        if not (i % save_at_every) and i > 0 and trainer.is_main and should_sample:
            images = trainer.sample(
                texts=[sample_texts], batch_size=1, return_pil_images=True, stop_at_unet_number=unet
            )
