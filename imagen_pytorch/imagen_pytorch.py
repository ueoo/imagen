    ...
    def __init__(
        ...
        *,
        ...
        self.to_time_tokens = nn.Sequential(
            nn.Linear(time_cond_dim, cond_dim * num_time_tokens), Rearrange("b (r d) -> b r d", r=num_time_tokens)
        )
        ...
        self.attn_pool = (
            PerceiverResampler(
                dim=cond_dim, depth=2, dim_head=attn_dim_head, heads=attn_heads, num_latents=attn_pool_num_latents
            )
            if attn_pool_text
            else None
        )
        ...
        self.init_resnet_block = (
            resnet_klass(init_dim, init_dim, time_cond_dim=time_cond_dim, use_gca=use_global_context_attn)
            if memory_efficient
            else None
        )
        ...
        for ind, (
            ...
            if not memory_efficient:
                post_downsample = (
                    ...
                    if not is_last
                    else Parallel(nn.Conv2d(dim_in, dim_out, 3, padding=1), nn.Conv2d(dim_in, dim_out, 1))
                )
        ...
        self.mid_attn = (
            TransformerBlock(mid_dim, depth=layer_mid_attns_depth, **attn_kwargs) if attend_at_middle else None
        )
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        ...
                        transformer_block_klass(
                            dim=dim_out, depth=layer_attn_depth, ff_mult=ff_mult, context_dim=cond_dim, **attn_kwargs
                        ),
...
class Imagen(nn.Module):
    def __init__(
        ...
        *,
        ...
        assert (
            tuple(sorted(temporal_downsample_factor, reverse=True)) == temporal_downsample_factor
        ), "temporal downsample factor must be in order of descending"
        ...
        *,
        ...
        if has_inpainting:
            ...
            inpaint_masks = self.resize_to(
                rearrange(inpaint_masks, "b ... -> b 1 ...").float(), shape[-1], **resize_kwargs
            ).bool()
        ...
        for times, times_next in tqdm(
            ...
            for r in reversed(range(resample_times)):
                ...
                if has_inpainting and not (is_last_resample_step or torch.all(is_last_timestep)):
                    ...
                    img = torch.where(self.right_pad_dims_to_datatype(is_last_timestep), img, renoised_img)
    ...
    def sample(
        ...
        if not self.unconditional:
            ...
            text_masks = default(text_masks, lambda: torch.any(text_embeds != 0.0, dim=-1))
        ...
        if exists(inpaint_images):
            ...
            assert not (
                self.condition_on_text and inpaint_images.shape[0] != text_embeds.shape[0]
            ...
            with context:
                ...
                if self.is_video and self.resize_cond_video_frames:
                    ...
                    video_kwargs = maybe_transform_dict_key(
                        video_kwargs, "post_cond_video_frames", temporal_downsample_fn
                    )
        ...
        **kwargs,
        ...
        if exists(random_crop_size):
            if is_video:
                ...
                x_start, lowres_cond_img, noise = map(
                    lambda t: rearrange(t, "b c f h w -> (b f) c h w"), (x_start, lowres_cond_img, noise)
                )
            ...
            if is_video:
                x_start, lowres_cond_img, noise = map(
                    lambda t: rearrange(t, "(b f) c h w -> b c f h w", f=frames), (x_start, lowres_cond_img, noise)
                )
        ...
        if self_cond and random() < 0.5:
            with torch.no_grad():
                ...
                x_start = (
                    noise_scheduler.predict_start_from_noise(x_noisy, t=times, noise=pred)
                    if pred_objective == "noise"
                    else pred
                )
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert (
            images.dtype == torch.float or images.dtype == torch.half
        ...
        if exists(prev_image_size):
            lowres_cond_img = self.resize_to(
                images, prev_image_size, **frames_to_resize_kwargs(prev_frame_size), clamp_range=self.input_image_range
            )
            lowres_cond_img = self.resize_to(
                lowres_cond_img,
                target_image_size,
