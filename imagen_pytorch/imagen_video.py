...
def Downsample(dim, dim_out=None):
    ...
    return nn.Sequential(
        Rearrange("b c f (h p1) (w p2) -> b (c p1 p2) f h w", p1=2, p2=2), Conv2d(dim * 4, dim_out, 1)
    )
...
class GlobalContext(nn.Module):
    ...
    def __init__(self, *, dim_in, dim_out):
        ...
        self.net = nn.Sequential(
            Conv2d(dim_in, hidden_dim, 1), nn.SiLU(), Conv2d(hidden_dim, dim_out, 1), nn.Sigmoid()
        )
...
class LinearAttentionTransformerBlock(nn.Module):
    def __init__(
        self, dim, *, depth=1, heads=8, dim_head=32, ff_mult=2, ff_time_token_shift=True, context_dim=None, **kwargs
    ):
        super().__init__()
        self.layers = nn.ModuleList([])
    ...
    def __init__(
        ...
        *,
        ...
        self.init_conv = (
            ...
            if init_cross_embed
            else Conv2d(init_channels, init_dim, init_conv_kernel_size, padding=init_conv_kernel_size // 2)
        )
        ...
        temporal_attn = lambda dim: RearrangeTimeCentric(
            Residual(
                Attention(dim, **{**attn_kwargs, "causal": time_causal_attn, "init_zero": True, "rel_pos_bias": True})
            )
        )
        ...
        for ind, (
            ...
            transformer_block_klass = (
                TransformerBlock if layer_attn else (LinearAttentionTransformerBlock if use_linear_attn else Identity)
            )
