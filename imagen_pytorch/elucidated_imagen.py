...
class ElucidatedImagen(nn.Module):
    ...
    def preconditioned_network_forward(
        self, unet_forward, noised_images, sigma, *, sigma_data, clamp=False, dynamic_threshold=True, **kwargs
    ):
    ...
    def one_unet_sample(
        ...
        **kwargs,
        ...
        gammas = torch.where(
            (sigmas >= hp.S_tmin) & (sigmas <= hp.S_tmax), min(hp.S_churn / hp.num_sample_steps, sqrt(2) - 1), 0.0
        )
        ...
        unet_kwargs = dict(
            sigma_data=hp.sigma_data, clamp=clamp, dynamic_threshold=dynamic_threshold, cond_scale=cond_scale, **kwargs
        )
        ...
        for ind, (sigma, sigma_next, gamma) in tqdm(
            ...
            for r in reversed(range(resample_times)):
                ...
                model_output = self.preconditioned_network_forward(
                    unet.forward_with_cond_scale, images_hat, sigma_hat, self_cond=self_cond, **unet_kwargs
                )
                ...
                if has_second_order_correction:
                    ...
                    model_output_next = self.preconditioned_network_forward(
                        unet.forward_with_cond_scale, images_next, sigma_next, self_cond=self_cond, **unet_kwargs
                    )
        ...
        for (
            ...
            with context:
                ...
                if self.is_video:
                    ...
                    video_kwargs = dict(
                        cond_video_frames=cond_video_frames, post_cond_video_frames=post_cond_video_frames
                    )
        ...
        if exists(random_crop_size):
            ...
            if is_video:
                images, lowres_cond_img = map(
                    lambda t: rearrange(t, "b c f h w -> (b f) c h w"), (images, lowres_cond_img)
                )
            ...
            if is_video:
                images, lowres_cond_img = map(
                    lambda t: rearrange(t, "(b f) c h w -> b c f h w", f=frames), (images, lowres_cond_img)
                )
